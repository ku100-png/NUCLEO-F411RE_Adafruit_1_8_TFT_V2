...
static void TFT_DisplayMenu(void)
{
  ...
  while ((tmp != JOY_DOWN) && (tmp != JOY_UP))
  {
    tmp = BSP_JOY_GetState();
		...
		if(tmp) BSP_LCD_DisplayStringAtLine(2, (uint8_t*)"       TMPP         ");
		
		/* Test buttons ABC */
		if(buttonstatus){
			if(buttonstatus == BUTTON_A){
				BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"       BUT_A        ");
			}else if(buttonstatus == (uint8_t)BUTTON_B){
				BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"       BUT_B        ");
			}else if(buttonstatus == BUTTON_C){
				BSP_LCD_DisplayStringAtLine(3, (uint8_t*)"       BUT_C        ");
			}
